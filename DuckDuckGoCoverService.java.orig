...
public class DuckDuckGoCoverService implements BookCoverProvider {
    ...
    @Override
    public void getCovers(CoverFetchRequest request, Consumer<CoverImage> consumer) {
        getCovers(request, () -> false, consumer);
    }

    public void getCovers(CoverFetchRequest request, BooleanSupplier isCancelled, Consumer<CoverImage> consumer) {
        try {
            ...
            log.error("Error in getCovers", e);
            throw ApiError.INTERNAL_SERVER_ERROR.createException("DuckDuckGo cover fetch failed");
        }
    }

    public void searchImages(String searchTerm, Consumer<CoverImage> consumer) {
        searchImages(searchTerm, () -> false, consumer);
    }

    public void searchImages(String searchTerm, BooleanSupplier isCancelled, Consumer<CoverImage> consumer) {
        try {
            if (isCancelled.getAsBoolean()) return;
            ...
            for (CoverImage img : images) {
                if (isCancelled.getAsBoolean()) return;
                consumer.accept(new CoverImage(img.getUrl(), img.getWidth(), img.getHeight(), index.getAndIncrement()));
            }
        } catch (APIException e) {
            throw e;
        } catch (Exception e) {
            log.error("Error in searchImages", e);
            throw ApiError.INTERNAL_SERVER_ERROR.createException("DuckDuckGo image search failed");
        }
    }
