...
public class GraalVmRuntimeHints implements RuntimeHintsRegistrar {
    ...
    public void registerHints(RuntimeHints hints, ClassLoader classLoader) {
        ...
        registerForeignFunctionHints(hints, classLoader);
    }
    ...
    private void registerForeignFunctionHints(RuntimeHints hints, ClassLoader classLoader) {
        // pdfium4j internal bindings
        List<String> pdfiumBindingClasses = Arrays.asList(
            "org.grimmory.pdfium4j.internal.AnnotBindings",
            "org.grimmory.pdfium4j.internal.BitmapBindings",
            "org.grimmory.pdfium4j.internal.DocBindings",
            "org.grimmory.pdfium4j.internal.EditBindings",
            "org.grimmory.pdfium4j.internal.TextBindings",
            "org.grimmory.pdfium4j.internal.ViewBindings"
        );

        // epub4j-native headers
        List<String> epubHeaderClasses = Arrays.asList(
            "org.grimmory.epub4j.native_parsing.EpubNativeHeaders"
        );

        for (String className : pdfiumBindingClasses) {
            registerDescriptorsFromClass(hints, classLoader, className);
        }

        for (String className : epubHeaderClasses) {
            registerDescriptorsFromClass(hints, classLoader, className);
            // Also scan inner classes for EpubNativeHeaders (generated by jextract-like tools)
            try {
                Class<?> clazz = classLoader.loadClass(className);
                for (Class<?> inner : clazz.getDeclaredClasses()) {
                    registerDescriptorsFromClass(hints, classLoader, inner.getName());
                }
    ...
    private void registerDescriptorsFromClass(RuntimeHints hints, ClassLoader classLoader, String className) {
        try {
            Class<?> clazz = classLoader.loadClass(className);
            for (Field field : clazz.getDeclaredFields()) {
                // Find all static final FunctionDescriptor fields
                if (Modifier.isStatic(field.getModifiers()) && field.getType().getName().equals("java.lang.foreign.FunctionDescriptor")) {
                    try {
                        field.setAccessible(true);
                        FunctionDescriptor desc = (FunctionDescriptor) field.get(null);
                        if (desc != null) {
                            String functionName = field.getName();
                            // In some jextract output, the descriptor is in a DESC field within a named inner class
                            if (functionName.equals("DESC") || functionName.equals("DESC$")) {
                                functionName = clazz.getSimpleName();
                            }
                            // Spring AOT registers Panama downcalls in the JNI category for GraalVM
                            hints.jni().registerFunction(functionName, desc);
                        }
                    } catch (Exception ignored) {
                        // Skip descriptors that can't be accessed or read
                    }
                }
            }
        } catch (ClassNotFoundException ignored) {
            // Skip classes not present in the current build context
        }
    }
