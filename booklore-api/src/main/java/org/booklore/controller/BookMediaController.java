...
public class BookMediaController {
    ...
    private ResponseEntity<Resource> serveResourceWithCaching(Resource resource, org.springframework.web.context.request.WebRequest request, String title) {
        try {
            if (resource.exists()) {
    ...
    public ResponseEntity<Resource> getBookThumbnail(
        ...
        org.booklore.model.entity.BookEntity book = bookService.getBookById(bookId);
        Resource resource = bookService.getBookThumbnail(bookId);
        return serveResourceWithCaching(resource, request, book.getMetadata().getTitle());
    }
